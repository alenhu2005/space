...
- 月相教材同步顯示太空位置與地面所見月面，地球上標示觀測者位置。教學模式顯示相對日下點位置與簡化太陽時；真實模式顯示實際座標與明示民用時區（預設台北，可自行選擇），不依經度推測時區。
...
目前版本包含 55 個單元測試（語句覆蓋率 99.31%、分支覆蓋率 95%）、125 個 Chromium／Firefox／WebKit 互動案例，以及 30 個 macOS 視窗尺寸視覺案例（含地球／月球縮放後追蹤及月食地表視角）。視覺基準刻意與 Darwin 平台綁定，避免不同作業系統的字型與 WebGL 差異造成誤報。
