...
- 月相教材同步顯示太空位置與地面所見月面；教學模式固定觀測者經緯度與選定太陽時，月相時間軸不再移動觀測者，並列出月升、中天、月落的約略太陽時。真實模式顯示實際座標、明示民用時區，以及 Astronomy Engine 計算的下一次月升、中天與月落。
...
目前版本包含 55 個單元測試（語句覆蓋率 99.31%、分支覆蓋率 94.9%）、125 個 Chromium／Firefox／WebKit 互動案例，以及 30 個 macOS 視窗尺寸視覺案例（含地球／月球縮放後追蹤及月食地表視角）。視覺基準刻意與 Darwin 平台綁定，避免不同作業系統的字型與 WebGL 差異造成誤報。
