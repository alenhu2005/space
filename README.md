...
- 太陽教材分為「當地太陽視運動」與「地球公轉與四季」兩個同步視窗；公轉視窗提供可旋轉、平移、縮放的自由視角，地球以平行太陽光與晨昏線顯示晝夜，並用青色緯圈標示觀測者隨地球自轉的每日軌跡。
- 太陽可切換原始 2D 舊版教材，返回新版時保留新版參數；左側場景選單可收合，手機收合後釋出模型空間。
- 三個主要教材皆支援游標位置縮放、雙指縮放與平移，另有地球／月球特寫；特寫跟隨天體移動，保留使用者調整的角度和倍率。
- 月相教材同步顯示太空位置與地面所見月面；教學模式將觀測者固定在選定經緯度，標記隨地球自轉且所在地太陽時會隨月相時間軸推進，並列出月升、中天、月落的約略太陽時。真實模式以小時連續推進實際地球自轉，顯示實際座標、明示民用時區，以及 Astronomy Engine 計算的下一次月升、中天與月落；切換模式時會對齊當前月相，避免畫面突然跳相。
...
目前版本包含 57 個單元測試（語句覆蓋率 98.67%、分支覆蓋率 93.63%）、130 個 Chromium／Firefox／WebKit 互動案例，以及 30 個 macOS 視窗尺寸視覺案例（含地球／月球縮放後追蹤及月食地表視角）。視覺基準刻意與 Darwin 平台綁定，避免不同作業系統的字型與 WebGL 差異造成誤報。
