...
export function createSunPath(context: BuildContext): SceneVisual {
  ...
  const ground = new THREE.Mesh(new THREE.CircleGeometry(4, 96), new THREE.MeshBasicMaterial({ color: 0x0a2029, side: THREE.DoubleSide }))
  ground.rotation.x = -Math.PI / 2
  const horizonBand = new THREE.Mesh(
    new THREE.TorusGeometry(4, .025, 8, 128),
    new THREE.MeshBasicMaterial({ color: COLORS.cyan, transparent: true, opacity: .82 })
  )
  ...
  // Keep the marker and latitude trail in the same local frame as the Earth texture.
  earthMini.add(observerTrail, observer, observerNormal)
  ...
  overlay.dataset.mobileView = 'local'
  overlay.innerHTML = `<div class="sun-mobile-tabs" role="tablist" aria-label="太陽同步模型">
    <button role="tab" data-sun-view="local" aria-controls="sun-local-panel" aria-selected="true">地面模型</button>
    <button role="tab" data-sun-view="space" aria-controls="sun-space-panel" aria-selected="false">公轉模型</button>
  </div>
  ...
  <section class="sun-view" id="sun-space-panel" data-sun-panel="space" role="region" aria-label="地球公轉與四季">
    ...
    <div class="earth-light-legend"><span>☀ 受光面</span><span>◐ 晨昏線</span><span class="night-side-key">背光面</span><span class="observer-trail-key">觀測者自轉軌跡</span><span class="earth-spin-key">↺ 北極上方看</span></div>
  ...
  const eclipticToSeasons = (vector: { readonly x: number; readonly y: number; readonly z: number }): THREE.Vector3 =>
    new THREE.Vector3(-vector.y, vector.z, -vector.x)
  const realEarthOrientation = (instant: Date): THREE.Quaternion => {
    const surfaceDirection = (latitude: number, longitude: number) => eclipticToSeasons(
      context.astronomy.observerVector(instant, { latitude, longitude, elevation: 0 })
    ).normalize()
    ...
    const textureWest = primeMeridian.clone().cross(north).normalize()
    return new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(primeMeridian, north, textureWest))
  }
  ...
  return {
    ...
    update(state) {
      ...
      let realSunVector: { readonly x: number; readonly y: number; readonly z: number } | undefined
      if (state.mode === 'real') {
        realSunVector = context.astronomy.geocentricVector('Sun', instant)
        seasonAngle = (radiansToDegrees(Math.atan2(realSunVector.y, realSunVector.x)) + 360) % 360
      }
      ...
      if (realSunVector) {
        earthTilt.position.copy(eclipticToSeasons(realSunVector).normalize().multiplyScalar(-orbitRadius))
        earthTilt.quaternion.copy(realEarthOrientation(instant))
        earthMini.rotation.set(0, 0, 0)
      } else {
        earthTilt.position.set(Math.cos(orbitalAngle) * orbitRadius, 0, -Math.sin(orbitalAngle) * orbitRadius)
        earthTilt.rotation.set(0, 0, degreesToRadians(23.44))
        earthMini.rotation.y = earthEastwardRotationRadians(state.timeline)
      }
      ...
      const hourAngle = state.mode === 'real'
        ...
        : (hour - 12) * 15
      const longitude = observerLongitudeRadians(Math.atan2(sunInEarthFrame.z, sunInEarthFrame.x), hourAngle)
      const latitudeRadians = degreesToRadians(latitude)
      const latitudeRadius = Math.cos(latitudeRadians)
      const desiredNormal = new THREE.Vector3(latitudeRadius * Math.cos(longitude), Math.sin(latitudeRadians), latitudeRadius * Math.sin(longitude))
      const textureDirection = earthTextureSurfaceDirection(latitude, state.observer.longitude)
      const normal = state.mode === 'real'
        ? new THREE.Vector3(textureDirection.x, textureDirection.y, textureDirection.z)
        : desiredNormal.applyQuaternion(earthMini.quaternion.clone().invert())
      ...
      const observerWorldNormal = normal.clone().applyQuaternion(earthMini.quaternion).applyQuaternion(earthTilt.quaternion)
      const modelSunAltitude = radiansToDegrees(Math.asin(Math.max(-1, Math.min(1,
        observerWorldNormal.dot(earthTilt.position.clone().negate().normalize())
      ))))
      ...
      const horizonState = position.altitude >= 0 ? 'above' : 'below'
      localReadout.dataset.horizon = horizonState
      localReadout.textContent = `太陽在地平面${horizonState === 'above' ? '上' : '下'} ${Math.abs(position.altitude).toFixed(1)}° · 緯度 ${latitude.toFixed(1)}° · 赤緯 ${signedDeclination}°`
      ...
      viewElements[0]!.dataset.horizonState = horizonState
      viewElements[0]!.dataset.polarisAltitude = String(latitude)
      viewElements[1]!.dataset.observerTrailLatitude = String(latitude)
      viewElements[1]!.dataset.observerTrailStyle = 'solid'
      viewElements[1]!.dataset.earthSpinDirection = 'counterclockwise-from-north'
      viewElements[1]!.dataset.observerTextureAlignment = 'true'
      viewElements[1]!.dataset.observerSunAltitude = modelSunAltitude.toFixed(2)
      viewElements[1]!.dataset.observerSunAltitudeError = Math.abs(modelSunAltitude - position.altitude).toFixed(2)
      return [
        ...
        { label: '正午高度公式', value: `90°−|${latitude.toFixed(1)}°−(${declination.toFixed(1)}°)| = ${noonAltitude.toFixed(1)}°` },
